...
const { fetchMeme, fetchWeebs, fetchHololive, fetchAnimeWall, fetchJapan, fetchDarkJoke } = require('./lib/fetcher')
...
async function msgHandler (client, message) {
    try {
        ...
        case 'help': {
            ...
            \n\n*Gif Sticker*\nCMD : $gif Giphy URL\nDescription: Convert gif to sticker (but giphy only), i mean provide the Giphy gif url.
            ...
            \n\nUnder development command 👨‍🔧 \n\nCMD: $weebs \nDescription: Send normal weebs pic like gbf, arknight etc.
            \n\nCMD: $hololive \nDescription: Hololive Update
            \n\nCMD: $animewall \nDescription: Please don't request it too much becuase of the image quality need more time to process.
            \n\nCMD: $japan \nDescription: Japan Life
            \n\nCMD: $darkjoke \nDescription: Of course darkjokes 👤
            \n\nCommand you won't too see 😨
            \n\nCMD: $preteenpus \nDescription: For ur daily *****`
        ...
        case 'gifstiker':
        case 'gifsticker':
            if (args.length !== 1) return client.reply(from, `Maaf ${pushname}, untuk saat ini sticker gif hanya bisa menggunakan link dari giphy.`, id)
            if (isGiphy) {
                ...
                await client.reply(from, `Maaf ${pushname}, untuk saat ini sticker gif hanya bisa menggunakan link dari giphy.`, id)
            }
            break
        // Video Downloader
        case 'tiktok':
        ...
        case 'hololive': {
            const { title, url } = await fetchHololive()
            await client.sendFileFromUrl(from, `${url}`, 'hololive.jpg', `${title}`)
            break
        }

        case 'japan': {
            const { title, url } = await fetchJapan()
            await client.sendFileFromUrl(from, `${url}`, 'japan.jpg', `${title}`)
            break
        }

        case 'darkjoke': {
            const { title, url } = await fetchDarkJoke()
            await client.sendFileFromUrl(from, `${url}`, 'darkjoke.jpg', `${title}`)
            break
        }

        case 'animewall': {
            const { title, url } = await fetchAnimeWall()
            await client.sendFileFromUrl(from, `${url}`, 'animwall.jpg', `${title} \n\nAnd Here's the download link: ${url}`)
            break
        }

        case 'preteenpus': {
            await client.sendText(from, 'Hayo mau lihat apa 🤭', id)
            break
        }
        ...
        default:
            ...
            await client.sendText(from, `Maaf ${pushname}, sepertinya anda memasukkan command yang tidak dikenal. Silahkan ketik *$menu* untuk bantuan. \nprefix = $`)
            break
        }
    } catch (err) {
